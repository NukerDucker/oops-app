...
def initialize_users():
    ...
    # Create hashed password oncerate_password_hash("password").decode('utf-8')
    hashed_password = bcrypt.generate_password_hash("password").decode('utf-8')
    
    # Admin user User(1, "Admin", hashed_password, "admin")
    admin_user = User(1, "Admin", hashed_password, "admin")ry")
    ...
    # Admin tasksd_task(Task(1, "Task 1", "This is task 1"))
    admin_user.add_task(Task(1, "Task 1", "This is task 1"))
    admin_user.add_task(Task(2, "Task 2", "This is task 2"))
    admin_user.add_task(Task(3, "Task 3", "This is task 3"))s is weekly task 1"))
    admin_user.add_weekly_task(Task(1, "Weekly Task 1", "This is weekly task 1"))
    admin_user.add_weekly_task(Task(2, "Weekly Task 2", "This is weekly task 2"))task 1"))
    admin_user.add_emergency_task(Task(1, "Emergency Task 1", "This is emergency task 1"))
    users.append(admin_user)
    
    # Doctor users User(2, "Doctor1", hashed_password, "doctor")
    doctor1_user = User(2, "Doctor1", hashed_password, "doctor")
    doctor1_user.add_access_permission("Patient", "/patient")
    doctor1_user.add_access_permission("Main", "/main")aul HIV checkup"))
    doctor1_user.add_task(Task(1, "Follow-Up", "Mr. Napaul HIV checkup"))scription"))
    doctor1_user.add_task(Task(2, "Prescribe", "Mr. Bento paracetamol prescription")) heartburn"))
    ...
    doctor1_user.add_weekly_task(Task(1, "Seminar @BKK", "Trans people seminar"))rotocols"))
    doctor1_user.add_weekly_task(Task(2, "Staff Meeting", "Discuss new hospital protocols"))ghtning"))
    ...
    doctor2_user.add_access_permission("Main", "/main")dwich toothache checkup"))
    doctor2_user.add_task(Task(1, "Follow-Up", "Mr. Sandwich toothache checkup"))er Ducker"))
    doctor2_user.add_task(Task(2, "Annual Physical", "Routine checkup for Mr. Nuker Ducker"))rol"))
    doctor2_user.add_task(Task(3, "Diet Consultation", "Help Mr. Eat Bento with high cholesterol"))
    doctor2_user.add_task(Task(4, "Vision Test", "Check Ms. Polly Graph's eyesight")) Nhow Socool"))
    ...
    doctor2_user.add_emergency_task(Task(1, "Euthanization", "Smoke you know who"))Outage in anaphylactic shock"))
    ...
    # Receptionist users User(4, "Sally Smith", hashed_password, "Receptionist")
...
def initialize_patients():
    ...
    # First set of patients (more detailed)it for chronic back pain - 2023-01-15")
    patient1 = Patient("John Doe", 45, "Male", "555-1234")in management - 2023-01-15")
    ...
    patients.append(patient1)Smith", 32, "Female", "555-5678")
    patient2.add_history_entry("Prenatal checkup, 24 weeks - 2023-03-05")
    patient2 = Patient("Jane Smith", 32, "Female", "555-5678")opment - 2023-03-05")
    patient2.add_history_entry("Prenatal checkup, 24 weeks - 2023-03-05")ed - 2023-04-02")
    ...
    # Additional patientsentry("Diagnosed with Type 2 Diabetes - 2022-11-12")
    patient3 = Patient("Robert Johnson", 67, "Male", "555-9012")2")
    patient3.add_history_entry("Diagnosed with Type 2 Diabetes - 2022-11-12")3-01-20")
    patient3.add_history_entry("Started on Metformin - 2022-11-12")mal - 2023-05-15")
    ...
    patients.append(patient3) Davis", 28, "Female", "555-3456")
    patient4.add_history_entry("Treatment for anxiety - 2023-02-18")
    patient4 = Patient("Emily Davis", 28, "Female", "555-3456")y - 2023-02-18")
    patient4.add_history_entry("Treatment for anxiety - 2023-02-18")
    patient4.add_history_entry("Prescribed Sertraline 50mg daily - 2023-02-18")
    patients.append(patient4)el Wilson", 52, "Male", "555-7890")
    patient5.add_history_entry("Hypertension diagnosis - 2022-09-30")
    patient5 = Patient("Michael Wilson", 52, "Male", "555-7890")30")
    patient5.add_history_entry("Hypertension diagnosis - 2022-09-30")ted - 2023-04-25")
    ...
    # Add more patients with creative names Treated for dehydration and muscle stiffness")
    patient6 = Patient("Anna Conda", 22, "Female", "Snake@hiss.com") throat", 
    ...
    patients.append(patient6) Outage", 45, "Female", "Manager@now.com")
...
def initialize_supplies():
    """Create mock medical supplies inventory"""100 tablets)", 50, 8.99, "Pain Relief"))
    global supplies(Supply("Amoxicillin (Box of 50 capsules)", 30, 12.50, "Antibiotic"))
    supplies.append(Supply("Insulin Vials (10ml each)", 45, 5.25, "Diabetes Management"))
    supplies.append(Supply("Paracetamol (Box of 100 tablets)", 50, 8.99, "Pain Relief")), "Cardiovascular"))
    supplies.append(Supply("Amoxicillin (Box of 50 capsules)", 30, 12.50, "Antibiotic"))ef"))
    supplies.append(Supply("Insulin Vials (10ml each)", 45, 5.25, "Diabetes Management"))
    ...
    supplies.append(Supply("Cough Syrup (Bottle of 250ml)", 20, 22.50, "Cold & Flu"))Treatment"))
...
def initialize_appointments():
    ...
    doctor2_id = 3  # Doctor2's ID timedelta(days=i)
    ...
    for i in range(1, 15):
        ...
        if i % 3 != 0:  # Skip every third day for variety
            ...
            appointments.append(Appointment(
                ...
                appt_date,
                time(10, 30)
            ))pointments.append(Appointment(
                patients[(i + 1) % 5].id,
        # Afternoon appointments% 2 == 0 else doctor2_id,
        if i % 2 == 0:  # Every other day
            appointments.append(Appointment(
                ...
                appt_date,
                time(14, 0)
            ))tments[0].update_status("completed")
        if len(appointments) > 1:
    # Set some appointments to different statusesled")
    if appointments:ntments) > 3:
        appointments[0].update_status("completed")")
        if len(appointments) > 1:
            appointments[1].update_status("cancelled")
        if len(appointments) > 3:e):
...
import routes.authintments
import routes.usersations
import routes.patientsls
import routes.inventory
import routes.appointments
import routes.medications
import routes.financialsypt
routes.users.users = users
# Share variables with blueprints
routes.auth.users = users= patients
routes.auth.bcrypt = bcrypters
routes.users.users = users= supplies
routes.patients.users = userssers
routes.patients.patients = patientsents
routes.inventory.users = usersts = appointments
routes.inventory.supplies = supplies
routes.appointments.users = usersients
routes.appointments.patients = patients
routes.appointments.appointments = appointments
routes.appointments.system = systemm
...
routes.financials.system = systemapp.register_blueprint(patients_bp)
t(inventory_bp)
# Register blueprintsents_bp)
app.register_blueprint(auth_bp)ns_bp)
app.register_blueprint(users_bp)p)
app.register_blueprint(patients_bp)
app.register_blueprint(inventory_bp)
app.register_blueprint(appointments_bp)
app.register_blueprint(medications_bp)app.register_blueprint(financials_bp)if __name__ == '__main__':    app.run(debug=True)
